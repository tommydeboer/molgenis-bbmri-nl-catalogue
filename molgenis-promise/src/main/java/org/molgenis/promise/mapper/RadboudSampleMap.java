...
class RadboudSampleMap
{
	...
	void addSample(Map<String, String> radboudSampleEntity)
	{
	...
	Iterable<Entity> getDataCategories(Map<String, String> radboudBiobankEntity)
	{
		...
		{ // get rest of data category ids from the biobank entity itself
			if ("1".equals(radboudBiobankEntity.get(XML_VOORGESCH)))
			{
				dataCategoryTypeIds.add("OTHER");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_FAMANAM)))
			{
				dataCategoryTypeIds.add("GENEALOGICAL_RECORDS");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_BEHANDEL)))
			{
				dataCategoryTypeIds.add("MEDICAL_RECORDS");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_FOLLOWUP)))
			{
				dataCategoryTypeIds.add("OTHER");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_BEELDEN)))
			{
				dataCategoryTypeIds.add("IMAGING_DATA");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_VRAGENLIJST)))
			{
				dataCategoryTypeIds.add("SURVEY_DATA");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_OMICS)))
			{
				dataCategoryTypeIds.add("PHYSIOLOGICAL_BIOCHEMICAL_MEASUREMENTS");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_ROUTINEBEP)))
			{
				dataCategoryTypeIds.add("PHYSIOLOGICAL_BIOCHEMICAL_MEASUREMENTS");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_GWAS)))
			{
				dataCategoryTypeIds.add("OTHER");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_HISTOPATH)))
			{
				dataCategoryTypeIds.add("OTHER");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_OUTCOME)))
			{
				dataCategoryTypeIds.add("NATIONAL_REGISTRIES");
			}

			if ("1".equals(radboudBiobankEntity.get(XML_ANDERS)))
			{
				dataCategoryTypeIds.add("OTHER");
			}
		}
	...
	private Set<String> collectDataCategoryIds(Map<String, String> radboudSampleEntity)
	{
		Set<String> dataCategoryTypeIds = newTreeSet();
		String deelbiobanks = radboudSampleEntity.get(XML_DEELBIOBANKS);
	...
	private Set<String> collectMaterialIds(Map<String, String> radboudSampleEntity)
	{
		Set<String> materialTypeIds = newTreeSet();

		if ("1".equals(radboudSampleEntity.get(XML_DNA)) || "1".equals(radboudSampleEntity.get(XML_DNABEENMERG)))
		{
			materialTypeIds.add("DNA");
		}

		if ("1".equals(radboudSampleEntity.get(XML_BLOED)))
		{
			materialTypeIds.add("WHOLE_BLOOD");
		}

		if ("1".equals(radboudSampleEntity.get(XML_BLOEDPLASMA)))
		{
			materialTypeIds.add("PLASMA");
		}

		if ("1".equals(radboudSampleEntity.get(XML_BLOEDSERUM)))
		{
			materialTypeIds.add("SERUM");
		}

		if ("1".equals(radboudSampleEntity.get(XML_WEEFSELSOORT)))
		{
			materialTypeIds.add("TISSUE_PARAFFIN_EMBEDDED");
		}
		else if ("2".equals(radboudSampleEntity.get(XML_WEEFSELSOORT)))
		{
			materialTypeIds.add("TISSUE_FROZEN");
		}

		if ("1".equals(radboudSampleEntity.get(XML_URINE)))
		{
			materialTypeIds.add("URINE");
		}

		if ("1".equals(radboudSampleEntity.get(XML_SPEEKSEL)))
		{
			materialTypeIds.add("SALIVA");
		}

		if ("1".equals(radboudSampleEntity.get(XML_FECES)))
		{
			materialTypeIds.add("FECES");
		}

		if ("1".equals(radboudSampleEntity.get(XML_RNA)) || "1".equals(radboudSampleEntity.get(XML_RNABEENMERG)))
		{
	...
	private Set<String> collectOmicsIds(Map<String, String> radboudSampleEntity)
	{
		Set<String> omicsTypeIds = newTreeSet();

		if ("1".equals(radboudSampleEntity.get(XML_GWASOMNI)) || "1".equals(radboudSampleEntity.get(XML_GWAS370CNV))
				|| "1".equals(radboudSampleEntity.get(XML_EXOOMCHIP)))
		{
	...
	private Set<String> collectSexIds(Map<String, String> radboudSampleEntity)
	{
		Set<String> genderTypeIds = newTreeSet();

		String genderValue = radboudSampleEntity.get(XML_GENDER);
	...
	private Integer collectAge(Map<String, String> radboudSampleEntity)
	{
		String birthDate = radboudSampleEntity.get(XML_BIRTHDATE);
		String inclusionDate = radboudSampleEntity.get(XML_INCLUSIE);
