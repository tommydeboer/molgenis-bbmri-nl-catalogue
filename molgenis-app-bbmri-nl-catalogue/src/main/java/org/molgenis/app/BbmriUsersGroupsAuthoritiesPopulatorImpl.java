...
import org.molgenis.file.ingest.FileIngesterPluginController;
...
@Service
public class BbmriUsersGroupsAuthoritiesPopulatorImpl implements BbmriUsersGroupsAuthoritiesPopulator
{
	private static final String DATA_EXPLORER_SETTINGS =
			SettingsPackage.PACKAGE_SETTINGS + PACKAGE_SEPARATOR + "dataexplorer";
	private static final List<String> DATA_MANAGER_ENTITY_READ_AUTHORITIES = Arrays
			.asList(FreemarkerTemplateMetaData.FREEMARKER_TEMPLATE, REF_DATA_CATEGORY_TYPES, REF_AGE_TYPES,
	...
	private static final List<String> DATA_MANAGER_ENTITY_WRITE_AUTHORITIES = Arrays
			.asList(REF_BIOBANKS, REF_JURISTIC_PERSONS, REF_PERSONS, REF_PUBLICATIONS, SAMPLE_COLLECTIONS_ENTITY);
	private static final List<String> DATA_MANAGER_PLUGIN_READ_AUTHORITIES = Arrays
			.asList(BackgroundController.ID, RedirectController.ID, ImportWizardController.ID);
	private static final List<String> DATA_MANAGER_PLUGIN_WRITE_AUTHORITIES = Arrays
			.asList(DataExplorerController.ID, FileIngesterPluginController.ID);
	private static final List<String> ALL_USERS_ENTITY_READ_AUTHORITIES = Arrays
			.asList(FreemarkerTemplateMetaData.FREEMARKER_TEMPLATE, LanguageMetadata.LANGUAGE, DATA_EXPLORER_SETTINGS);
	private static final List<String> ALL_USERS_PLUGIN_READ_AUTHORITIES = Arrays
			.asList(HomeController.ID, RedirectController.ID);
	private static final List<String> ALL_USERS_PLUGIN_WRITE_AUTHORITIES = singletonList(UserAccountController.ID);
	private static final List<String> ANONYMOUS_USER_PLUGIN_READ_AUTHORITIES = Arrays
			.asList(HomeController.ID, RedirectController.ID, FeedbackController.ID, DataExplorerController.ID);
	private static final List<String> ANONYMOUS_USER_ENTITY_READ_AUTHORITIES = Arrays
			.asList(FreemarkerTemplateMetaData.FREEMARKER_TEMPLATE, REF_AGE_TYPES, REF_BIOBANKS, REF_COLLECTION_TYPES,
					REF_COUNTRIES, REF_DATA_CATEGORY_TYPES, REF_DISEASE_TYPES, REF_EXP_DATA_TYPES, REF_JURISTIC_PERSONS,
					REF_MATERIAL_TYPES, LanguageMetadata.LANGUAGE, REF_PERSONS, REF_PUBLICATIONS,
					SAMPLE_COLLECTIONS_ENTITY, REF_SAMPLE_SIZE_TYPES, REF_SEX_TYPES, REF_STAFF_SIZE_TYPES,
	...
	@Autowired
	BbmriUsersGroupsAuthoritiesPopulatorImpl(DataService dataService, GroupFactory groupFactory,
			GroupAuthorityFactory groupAuthorityFactory, UserAuthorityFactory userAuthorityFactory)
	{
	...
	public void populate()
	{
		...
		dataService.add(GROUP, dataManagerGroup);
		dataService.add(GROUP_AUTHORITY, groupAuthorities.stream());
		dataService.add(USER_AUTHORITY, userAuthorities.stream());
	}
	...
	private List<UserAuthority> createUserAuthorities(User anonymousUser)
	{
		...
		ANONYMOUS_USER_ENTITY_READ_AUTHORITIES.stream()
				.map(entity -> createUserAuthority(anonymousUser, AUTHORITY_ENTITY_READ_PREFIX + entity))
				.forEach(userAuthorities::add);

		ANONYMOUS_USER_PLUGIN_READ_AUTHORITIES.stream()
				.map(plugin -> createUserAuthority(anonymousUser, AUTHORITY_PLUGIN_READ_PREFIX + plugin))
				.forEach(userAuthorities::add);
	...
	private List<GroupAuthority> createGroupAuthorities(Group dataManagerGroup, Group allUsersGroup)
	{
		...
		DATA_MANAGER_ENTITY_READ_AUTHORITIES.stream()
				.map(entity -> createGroupAuthority(dataManagerGroup, AUTHORITY_ENTITY_READ_PREFIX + entity))
				.forEach(groupAuthorities::add);

		DATA_MANAGER_ENTITY_WRITE_AUTHORITIES.stream()
				.map(entity -> createGroupAuthority(dataManagerGroup, AUTHORITY_ENTITY_WRITE_PREFIX + entity))
				.forEach(groupAuthorities::add);

		DATA_MANAGER_PLUGIN_READ_AUTHORITIES.stream()
				.map(plugin -> createGroupAuthority(dataManagerGroup, AUTHORITY_PLUGIN_READ_PREFIX + plugin))
				.forEach(groupAuthorities::add);

		DATA_MANAGER_PLUGIN_WRITE_AUTHORITIES.stream()
				.map(plugin -> createGroupAuthority(dataManagerGroup, AUTHORITY_PLUGIN_WRITE_PREFIX + plugin))
				.forEach(groupAuthorities::add);

		ALL_USERS_ENTITY_READ_AUTHORITIES.stream()
				.map(entity -> createGroupAuthority(allUsersGroup, AUTHORITY_ENTITY_READ_PREFIX + entity))
				.forEach(groupAuthorities::add);

		ALL_USERS_PLUGIN_READ_AUTHORITIES.stream()
				.map(plugin -> createGroupAuthority(allUsersGroup, AUTHORITY_PLUGIN_READ_PREFIX + plugin))
				.forEach(groupAuthorities::add);

		ALL_USERS_PLUGIN_WRITE_AUTHORITIES.stream()
				.map(plugin -> createGroupAuthority(allUsersGroup, AUTHORITY_PLUGIN_WRITE_PREFIX + plugin))
				.forEach(groupAuthorities::add);
	...
	private GroupAuthority createGroupAuthority(Group group, String role)
	{
		...
		groupAuthority.setRole(role.toUpperCase());
	...
	private UserAuthority createUserAuthority(User user, String role)
	{
		...
		userAuthority.setRole(role.toUpperCase());
